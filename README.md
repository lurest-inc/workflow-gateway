...
### 既存ワークフローからの移行 {#migration-guide}

以前の個別ワークフロー（`claude-exec-issue.yml`, `claude-fix-pr.yml`, `claude-interactive.yml`）を使用している場合:

1. 既存のワークフローファイルを削除
2. 上記の `claude-gateway.yml` を使用する設定に置き換え
3. `LUREST_DISPATCH_TOKEN` のSecretはそのまま使用可能
